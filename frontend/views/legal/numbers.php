...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="px-4 py-3 border-b border-gray-200 sm:px-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900">Договiр № <?= $num ?>:</h3>
    </div>
    ...
    foreach ($model as $item): ?>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-3 border-b border-gray-200 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Номер засобу облiку
                    води: <?= $item->accounting_number ?> </h3>
            </div>
            <div>
                <dl>
                    ...
                    <div class="bg-gray-50 px-2 py-1 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm leading-5 font-medium text-gray-500">Теперiшнi показання:</dt>
                    ...
                    <div class="bg-gray-50 px-2 py-1 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <?= $item->date_readings
                                ? Yii::$app->formatter->asDate($item->date_readings, 'php: d.m.Y')
                                : '' ?>
                        </dd>
                    </div>
                    <div class="bg-gray-50 px-2 py-1 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <?= $item->verification_date
                                ? Yii::$app->formatter->asDate($item->verification_date, 'php: d.m.Y')
                                : '' ?>
                        </dd>
                    </div>
            ...
            <div class="mt-6">
                ...
                if (strtotime($item->date_readings) !== strtotime(date('Y-m-d'))):?>
                    <a type="button"
                       href="<?= \yii\helpers\Url::to(['/legal/meter', 'num' => $num, 'acc' => $item->accounting_number]) ?>"
